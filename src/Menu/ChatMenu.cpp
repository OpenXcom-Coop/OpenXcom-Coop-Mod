	...
	if (event.type == SDL_KEYDOWN)
	{
		...
		else if (event.key.keysym.sym == SDLK_RETURN)
		{
			if (!inputText.empty())
			{
				...
				msg.player = _game->getHostName();
				msg.text = inputText;

				if (_game->getCoopStatic())
				{
					Json::Value root;
		...
		{
			// Convert single character to UTF-8 using OpenXcom Unicode
...
void ChatMenu::draw(SDL_Surface* screen)
{
	...
	int ping = _game->getPing();
	if (ping != -1)
	{
		std::string latencyText = "Latency: " + std::to_string(ping) + " ms";
