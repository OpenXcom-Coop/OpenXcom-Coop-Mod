 ...
 *
 * In a SHARED campaign there is a single host-authoritative world; clients hold
 ...
 *
 *   shared_cmd   client -> host   "please apply this mutation" (client mutates
 *                                 nothing locally)
 *   shared_ok    host   -> client "accepted" (correlated by seq)
 *   shared_fail  host   -> client "rejected: reason"
 *   shared_apply host   -> ALL    "validated + applied; here is the mutation and
 *                                 the new authoritative funds" (replicas apply
 *                                 from this ONLY)
 *
 * Flow (client-originated): client submitLocalCmd() -> shared_cmd -> host
 *   validates on the MAIN thread, debits funds, applies, broadcasts shared_apply
 *   to all + shared_ok to the initiator. The client applies from shared_apply.
 * Flow (host-originated): host submitLocalCmd() -> validate+apply locally +
 *   broadcast shared_apply.
 *
 * Every shared_apply carries the post-mutation authoritative getFunds(); replicas
 * setFunds(funds) on apply, so funds self-heal per mutation. The SEPARATE
 * coopFunds peer-mirror is fenced off in SHARED and stays fenced.
 *
 * Anti-if-chain: shared_* messages are NOT appended to the onTCPMessage if-chain.
 ...
 * untouched; the initiator gets shared_fail). @a base is the resolved target, or
 ...
 * (after a passing validate) AND on every REPLICA (from shared_apply). MUST be
...
// shared_apply landing while such a screen is open leaves it stale. There is ONE
...
// (post-validate) AND on every replica (from shared_apply).
//
...
/// Index of @a base in SavedGame::getBases() - the SHARED shared base key - or -1.
int baseIndex(Game* game, const Base* base);

/**
 * PRD-J10 screen-refresh binding. A SHARED screen owns one of these, binds it in
...
{
public:
	/// Bind to the apply stream. No-op unless this is a SHARED campaign, so callers
	...
	bool consume();
	/// Is this screen bound (i.e. a live SHARED refresh)?
...
/// was a shared_* protocol message (and was consumed). Main thread.
...
// In SHARED only the host runs world simulation; replicas are frozen (their
// timeXxx handlers early-return). Each host-only completion event is therefore
// mirrored to replicas over the SAME registerCmd/shared_apply channel: the host
...
// double-apply). Each helper is a no-op unless isSharedCampaign() && the caller is
...
// Any player commands any craft; orders ride shared_cmd (craft_launch /
...
// reaches replicas via the existing shared:true position snapshot; there is no
// per-fight result message and no snapshot exemption.

/// UI entry points (initiator side; SHARED only - caller gates). Each builds the
/// payload and submits the matching shared_cmd; nothing is mutated locally.
...
/// PRD-J09: assign/remove @a soldier to/from @a craft in the shared world (SHARED
/// only - caller gates). @a onOff = desired final state (true = aboard). Submits
/// craft_assign; mutates nothing locally (replica applies from shared_apply).
...
/// (SHARED only - caller gates). The old armor's store item is returned + the new
...
/// the shared_apply lane via registerCmd and needs no public entry point.)
...
/// Generic informational-alert replication (HOST -> all seats). In SHARED only the host
/// runs the geoscape sim, so every popup it raises is invisible to the frozen replicas.
/// Instead of a bespoke shared_cmd per dialog, name the dialog class plus whatever ids /
...
/// decision to the host (shared_cmd{land_reply}); the host owns the consequence.
...
// the replica asks for a fresh world (shared_resync_request), the host re-streams
...
/// The checksum and the shared_apply that moves it are separate packets, so an
...
struct ResyncStats
{
	...
	uint64_t requests;    // shared_resync_request sent (replica) / served (host)
...
/// process has processed, plus the most recent shared_fail reason surfaced here.
struct Stats
{
	uint64_t cmd;     // shared_cmd validated by this host
	uint64_t ok;      // shared_ok received (initiator) or sent (host)
	uint64_t fail;    // shared_fail surfaced (initiator) or sent (host)
	uint64_t apply;   // shared_apply applied by this replica / host
	uint64_t unknown; // shared_cmd with an unregistered cmd string
};
Stats stats();
