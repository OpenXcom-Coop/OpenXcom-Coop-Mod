...
$bad = @($names | Where-Object { $_ -match '^(UFO|TFTD)/' -and $_ -notmatch '^(UFO|TFTD)/(multiplayer|README)/' })
...
Write-Host "package OK ($Archive): coop art + rendezvous.json + LICENSE.txt + HOW_TO_RUN.txt present, no licensed retail data ($($names.Count) entries)"
