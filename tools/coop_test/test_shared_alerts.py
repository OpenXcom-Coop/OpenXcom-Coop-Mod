"""Bug: geoscape alerts don't reach the CLIENT in a SHARED campaign (reported for "UFO
lost", but it was systemic - ~30 informational popups were host-only).

In SHARED only the host runs the geoscape sim (every timeXxx handler early-returns for a
...
Fix: ONE generic `alert` shared_cmd. The host names the dialog class plus the ids / rule
names needed to rebuild it (SharedEcon::hostAlert), and a replica-side factory
...
Run:  python tools/coop_test/test_shared_alerts.py
"""
...
def main():
    js = shared_fixture.bring_up("jalert", (48904, 48905, 48204))
    ...
    try:
        ...
        js.finish()
        print("ALL SHARED ALERT-REPLICATION TESTS PASSED")
    finally:
        js.shutdown()
