"""PRD-J06: SHARED manufacture start / allocate / cancel via shared_cmd.

A SHARED campaign is one host-authoritative world. The manufacture screens mutate
NOTHING locally: starting a production (first-unit funds + material debit),
(re)allocating engineers/qty, and cancelling each ride the J03 shared_cmd
protocol. The host validates against the live world, applies, and broadcasts
shared_apply (carrying the new authoritative funds); replicas apply ONLY from
shared_apply. Completion is host-driven (J04 prod_done).
...
Run:  python tools/coop_test/test_shared_manufacture.py
"""
...
def main():
    js = shared_fixture.bring_up("jman", (48730, 48731, 48030))
    ...
    try:
        ...
        client.ok({"cmd": "shared_cmd", "jcmd": "man_cancel", "baseId": 0,
        ...
        # ================================================================
        # 5) DAYS-LEFT sync: a SHARED replica's timeXxx handlers are frozen, so a
        ...
        print("ALL SHARED MANUFACTURE TESTS PASSED")
    finally:
        js.shutdown()
