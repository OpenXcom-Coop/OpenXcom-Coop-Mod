...
def drive_to_battlescape(host, client, seated_holder, seat_count=2):
    ...
    # this click (turnMode itself is still resolved at PREPARE time, unmoved).
...
if __name__ == "__main__":
    try:
        ...
        session.print_known_flake_banner("test_rw_turn_mode", e.tracking, str(e))
        print(f"\ntest_rw_turn_mode: FAIL (KNOWN FLAKE, evidence recorded)\n{e}")
        sys.exit(2)
    except AssertionError as e:
        if str(e).startswith("FIXTURE:"):
            print(f"\ntest_rw_turn_mode: SKIP (fixture) - {e}")
            sys.exit(3)
        print(f"\ntest_rw_turn_mode: FAIL\nAssertionError: {e}")
        sys.exit(2)
