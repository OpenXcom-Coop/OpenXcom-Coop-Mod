...
test_shared_battle.py owns that half; this test owns the routing.
...
Run:  python tools/coop_test/test_shared_landing.py
"""
...
def _pending(gc):
    return gc.ok({"cmd": "shared_landing_state"})["pending"]
...
def _fly_to_site(host, cid, site_id, blon, blat):
    ...
    craft_launch shared_apply already recorded on both machines."""
...
def main():
    js = shared_fixture.bring_up("jland", (48770, 48771, 48070))
    ...
    try:
        ...
        #    recorded from the craft_launch shared_apply on BOTH machines.
        # ================================================================
        ...
        print("ALL SHARED LANDING TESTS PASSED")
    finally:
        js.shutdown()
