    ...
    for u in units.values():
        if u.get("soldierId") == soldier_id:
            if has_door_within(host, u["x"], u["y"], u["z"], radius=2):
                return None  # rule (b)
            if not session.actor_is_contact_free(st, u, "kneel"):
                return None  # rule (c)
            return u
    return None
...
if __name__ == "__main__":
    try:
        ...
        session.print_known_flake_banner("repro_atom_kneel", e.tracking, str(e))
        print(f"\nrepro_atom_kneel: FAIL (KNOWN FLAKE, evidence recorded)\n{e}")
        sys.exit(2)
    except AssertionError as e:
        if str(e).startswith("FIXTURE:"):
            print(f"\nrepro_atom_kneel: SKIP (fixture) - {e}")
            sys.exit(3)
        print(f"\nrepro_atom_kneel: FAIL\nAssertionError: {e}")
        sys.exit(2)
