"""PRD-J11: shared bring-up + world-equality assertion for every SHARED test.

Two things every SHARED test needs, in one place:

  bring_up(tag, ports)          - "host creates a SHARED campaign, client joins,
                                  world streamed, both settled on the geoscape".
                                  Returns a SharedSession (host/client/dirs).
  assert_world_equal(h, c, tag) - the SHARED promise, asserted: ONE shared world.
                                  Deep-compares an introspection dump of both
                                  machines, modulo the known-volatile fields
                                  below. Wired into every SHARED test's final
                                  state, so any future test gets it for free.

Typical use (matches the existing suite's flat try/finally shape):

    js = shared_fixture.bring_up("jbuy", (48670, 48671, 47970))
...
--------------------
The SHARED guarantee is one host-authoritative ECONOMY world: funds, bases,
...
of the design, not a fudge):
  ...
  * clock (hour/minute)         - the replica's clock is host-driven and lands a
                                  ...
                                  asserted by test_shared_sim / test_shared_month_run.
  ...
  * ufos + missionSites         - snapshot-replicated best-effort: replica despawn
                                  ...
                                  UFO/site visibility is asserted by test_shared_sim
                                  and test_shared_craft directly.

Equality is EVENTUAL: assert_world_equal polls, because a shared_apply in flight
...
def world_dump(gc):
    ...
    return {
        ...
        # authoritative (the shared_apply carries them; the replica adopts them
...
def assert_world_equal(host, client, tag="", timeout=45, interval=1.0):
    """THE SHARED invariant: host and replica hold one identical shared world.

    Polls until equal (an in-flight shared_apply is a legitimate transient skew -
...
class SharedSession:
    ...
    def finish(self, timeout=45):
        """Standard SHARED end-of-test assertions: the two worlds are one, and the
        replica never touched the disk. Call at the end of every SHARED test."""
