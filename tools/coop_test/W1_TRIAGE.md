...
| `test_skirmish_flow.py` | **UNBLOCKED - RUNS IN FULL (W1-P3)** | **RE-POINTED IN W1-P1, CLEARED IN W1-P3.** W1-P1: the stale `SKIP-PENDING(R4-P1)` guard sat at MODULE scope, two lines above the file's own `sys.path.insert`, so `sys.exit(0)` fired at IMPORT time - and `pvp_fixture.py:19` does `import test_skirmish_flow as SK`, so every `pvp_fixture` importer died silently with EXIT CODE 0 before its first assertion. W1-P1 removed the module guard and re-ran the file at `8c53c2592`: steps 1-6 PASS; step 7 failed on one assertion - `client left sitting on a dead lobby it cannot dismiss:` `['MainMenuState', 'NewBattleState', 'ServerList', 'LobbyMenu', 'BattlescapeState']` - which was the CURRENT client battle-entry shape, not a stale assumption (the client entry pushed a bare BattlescapeState over the client's whole stack and never tore the lobby down). W1-P3 (D3 / WV-D9) converges the client's entry onto the host's: it unwinds the client's pre-battle menu stack via the existing bounded `coopUnwindToSafeState()` and pushes BattlescapeState + a read-only BriefingState over it, so the client now lands on `['MainMenuState', 'BattlescapeState', 'BriefingState']`. `SKIP_FULL_FLOW = False` since W1-P3; **3/3 scenarios PASS** with the step-7 assertion UNCHANGED |
