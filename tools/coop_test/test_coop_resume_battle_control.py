...
on-screen for the whole walk. W2-H3 (F465, D137, D140): vanilla paces only an
on-screen walk (911ca487f UnitWalkBState.cpp:100 `onScreen`, :279 interval 0
off-screen); a re-centred click left the walker off-screen, the walk ran
unpaced and could end before the save request arrived. Lifted from the
...
def stage_alien_and_walk(host, client, walker_id):
    ...
    W2-H3 (F465, D137, D140): vanilla paces a walk only while the walker is on
    screen (911ca487f UnitWalkBState.cpp:100/:279). If the destination is off
    the HOME view, map_tile_click_pos re-centres the camera on it
