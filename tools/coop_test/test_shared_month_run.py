"""PRD-J11: the SHARED long-run - two month boundaries with a battle in between.

Every other SHARED test proves ONE mechanism in isolation, from a fresh bootstrap.
This one is the closest thing the suite has to a playthrough: a single SHARED
...
DIFFERENT channel, and this is the only test where they overlap in one campaign.
  ...
  phase 1  research + manufacture   -> client-originated shared_cmd (J06)
  phase 2  MONTH END #1             -> the monthly settlement rides the extended
                                       monthly_report packet, NOT shared_apply (J04)
...
Run:  python tools/coop_test/test_shared_month_run.py
"""
...
def _roll_month(host, client, tag):
    ...
    # Every shared_apply carries the host's _incomes.back()/_expenditures.back() and
    ...
    # (Pre-fix this drifted net-vs-gross and was only printed; test_shared_graphs is
...
def _fly_a_battle(host, client):
    ...
    def _prompt():
        if _has(host, "ConfirmLandingState"):
            return True
        # Unlike test_shared_battle (which flies from a fresh bootstrap), this craft
...
def main():
    t_start = time.time()
    js = shared_fixture.bring_up("jmonth", (48800, 48801, 48100))
    ...
    try:
        ...
        #          test_shared_battle, which flies from a fresh bootstrap.
        # ================================================================
        ...
        print(f"ALL SHARED MONTH-RUN TESTS PASSED (2 month ends + 1 battle, "
              f"{mins:.1f} min)")
    finally:
        js.shutdown()
