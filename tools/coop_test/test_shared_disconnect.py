"""PRD-J11: a SHARED client dropping mid-session, including with a command in flight.

The SHARED world lives on the host, so losing a client must cost nothing: the
...
through the ordinary resume path (PRD-J02's streamer).
  ...
  INFLIGHT   a shared_cmd is submitted and the client is HARD-KILLED microseconds
...
Run:  python tools/coop_test/test_shared_disconnect.py
"""
...
def main():
    # NOTE: like test_shared_resume, this test outlives its fixture - the client is
    # killed and a DIFFERENT client process rejoins - so the second client is owned
    # by hand.
    js = shared_fixture.bring_up("jdis", (48790, 48791, 48090))
    host, client = js.host, js.client
    try:
        shared_fixture.assert_world_equal(host, client, "bootstrap")
        ...
        shared_fixture.assert_world_equal(host, client, "after the baseline buy")
        ...
        print("kill: client hard-killed with a shared_cmd on the wire")
        ...
        assert cm.get("campaignType") == 1, f"rejoined client is not SHARED: {cm}"
        shared_fixture.assert_world_equal(host, client, "after rejoin", timeout=90)
        ...
        client.wait_for("rejoined client applied the shared_apply",
                        lambda: _rifles(client) == r3 + 4 or None, timeout=30, interval=0.5)
        shared_fixture.assert_world_equal(host, client, "after the post-rejoin buy")
        ...
        print("ALL SHARED DISCONNECT TESTS PASSED")
    finally:
        host.shutdown()
        client.shutdown()
