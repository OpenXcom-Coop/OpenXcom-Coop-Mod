"""PRD-J09 GAP-1 - SHARED base defense uses the ownership control split.
...
loadWorld(111)), which in SHARED forces EVERY shared soldier to host control (its
...
This test proves the shared base's garrison fights split BY OWNERSHIP:
  ...
  3. No soldier duplication: every deployed soldier-unit id is a real roster id
     ...
     ids).
  4. After the battle both worlds stay IDENTICAL (the SHARED post-battle merge /
...
build a SECOND shared base (via the real SHARED base_new flow): losing the
...
Run:  python tools/coop_test/test_shared_base_defense.py
...
def _roster(gc):
    """Every soldier at the shared base(s), flattened; SHARED has one world."""
    out = []
...
def main():
    js = shared_fixture.bring_up("jbasedef", (48774, 48775, 48074))
    ...
    try:
        ...
        #      savable (and non-game-over). Real SHARED base_new flow -> both sides.
        ...
        # ---- both machines enter the SHARED base-defense battle ---------------
        for gc, tag in ((host, "host"), (client, "client")):
        ...
        print("PASS entry: BOTH machines entered the SHARED base defense from the shared world")
