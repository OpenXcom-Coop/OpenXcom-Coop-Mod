...
| STR_COOP_REACTIONS_HOST_ONLY | INERT | - | RETIRED on the client by W2-P4 S-D (owner D133 (a); spec `rewrite/prompts/w2p4_client_combat_intents.md` Q13 = (a), RB-D3's "unused keys are inert" precedent, the `STR_COOP_TURN_OVER` row's shape): the client's hand right-click now sends a `reaction_hands` intent the host runs with vanilla's own toggle (`coopInterceptReactionHands()`), so the client never reaches this string. `Control::HandReaction` stays for its HOST ownership term, which shows `STR_COOP_DENY_NOT_YOUR_UNIT`; `controlStrKey()` still maps the value to this key, but only the client-side authority term ever presented it. The `.yml` value STAYS (both files). Was: MINTED + WIRED by W1-P5 (WV-D14). The right-click branch of `BattlescapeState::btn{Left,Right}HandItemClick` -> `toggle{Left,Right}HandForReactions()`. Those fields (`preferredHandForReactions`, `reactionsDisabledFor{Left,Right}Hand`) are serialized (BattleUnit.cpp:791-796) and are NOT on `saveBlobExcludedUnitKey`'s list, so a client toggle diverged saveBlob immediately - proven by `test_rw_client_gates.py`'s phase-3 negative control. These are evidence F2's "open item 9" fields; W1-P15's audit gives them a bucket home. |
