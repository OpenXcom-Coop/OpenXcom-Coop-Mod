...
| KEY | STATE | OWNER | DISPOSITION |
|---|---|---|---|
| STR_COOP_TURN_OVER | INERT | - | RETIRED by W1-P13b (WR-3's LIFETIME rule, as this row itself predicted). Was RE-VALUED + WIRED by W1-P7 (SS2.W8 / WV-D23 / ruling D-10): value REPLACED from "The turn has already ended" (factually wrong at the only place it was reachable) to "Only the host can end the turn", raised by `CoopBattleUi::showEndTurnHostOnly()` from `BattlescapeState::btnEndTurnClick`. W1-P13b (SPEC 10 / SS2.W3) removed that presenter entry and its ONLY call site entirely: the client's END TURN press now ARMS the readiness tally (`bt_end_turn_ready`) instead of refusing, so the branch this key covered has no reachable case left. Per RB-D3's "unused keys are inert" precedent the `.yml` value STAYS (untouched, both `en-US.yml`/`en-GB.yml`) but is now unreferenced from `src/`. DISTINCT from STR_COOP_DENY_TURN_OVER, which keeps its own text and its own (unrelated) wire deny reason. |
...
| STR_COOP_END_TURN_TALLY | WIRED | - | WIRED by W1-P13b (D1 / WV-D7 / SS2.W3). Painted onto `_txtCoopEndTurn` via `BattlescapeState::setCoopEndTurnText()`, fed by `src/CoopMod/CoopEndTurn.h`'s readiness-tally engine: `tr("STR_COOP_END_TURN_TALLY").arg(count).arg(needed)` renders exactly `"END TURN {0}/{1}"` (e.g. count 1 of needed 2 -> `"END TURN 1/2"`), hidden (empty string) whenever this machine's own tally count is 0. No `.yml` edit was needed - `en-US.yml`/`en-GB.yml` already carried the correct value from W1-P1's mint. |
...
| STR_COOP_DENY_CANNOT_USE | WIRED | - | MINTED + WIRED by W2-P4 S-A (spec `rewrite/prompts/w2p4_client_combat_intents.md` (b)2, orchestrator ruling Q1 = (a)): the combat admission deny `cannot_use`, `kReasonStrTable` row. The host denies it when `SavedBattleGame::canUseWeapon()` refuses with no message (AI delay, psi, mana) or the weapon has no configuration for the ordered action; every other combat deny reuses vanilla's own key, so this is the one reason vanilla has no text for. |
...
| STR_COOP_WAITING_FOR_JOIN | WIRED | - | WIRED (FX-1 / WV-D56) - host input freeze banner. MINTED + WIRED by FX-1 (WAVE1-RUNBOOK.md REV E.1, WV-D56). Raised by `CoopBattleUi::freezeBattleInputUntilActive()` (called from the top of `BattlescapeState::handle`) each time it refuses a HOST input while the battle handshake has not yet reached phase Active - the window WV-D56 opens by moving the coop blob snapshot + `battle_offer` send to AFTER `SavedBattleGame::startFirstTurn()` (`CoopHandshake::emitPreparedOffer()`), so the host now enters `BattlescapeState` before the client holds the blob. Notice class, never Sticky (would block every later banner for the rest of the battle). Like `STR_COOP_EQUIP_FROZEN` it explains an absence (no client has joined the handshake yet), not a refused press. |
...
| STR_COOP_LEVEL_CHANGE_HOST_ONLY | WIRED | - | MINTED + WIRED by W1-P13d (WAVE1-RUNBOOK.md SPEC 12 / WV-D58). `BattlescapeState::btnUnitUpClick` / `btnUnitDownClick` -> `BattlescapeGame::moveUpDown()`, which pushes a `UnitWalkBState` - a LOCAL walk on a client, bypassing every coop gate (WV-D40). Gated via `CoopBattleUi::refuseControl(Control::LevelChange)` on the two buttons; the host's own press routes through `coopInterceptWalkConfirm()` in `moveUpDown()`, the same walk-emit path used elsewhere. |
| STR_COOP_ITEM_ACTION_HOST_ONLY | WIRED | W2-P4 | MINTED + WIRED by W2-P1 (thin-client tripwire, plan F422/F432, SS3.6). `ActionMenuState::handleAction` -> `CoopBattleUi::refuseItemActionChoice()` -> `refuseControl(Control::ItemAction)` for the five NON-TARGETING kinds (`BA_PRIME`, `BA_UNPRIME`, `BA_HIT`, `BA_USE` on a `BT_MEDIKIT` or `BT_SCANNER`): prime/unprime, the medikit, the motion scanner and a melee attack each ran vanilla locally on a client (fuse, TU, target health/stun/wounds, item charges, a local `MeleeAttackBState`) with nothing on the wire. INTERIM: RETIRED by W2-P4 when its intents replace the refusal (the key and this row are deleted then). |
| STR_COOP_RELOAD_HOST_ONLY | WIRED | W2-P4 | MINTED + WIRED by W2-P1 (thin-client tripwire, plan F422/F432, SS3.6). `BattlescapeState::btnReloadClick` (the reload hotkey) -> `refuseControl(Control::Reload)`: `BattleUnit::reloadAmmo()` re-linked ammo (`items`) and spent TU (`unitsStats`) locally on a client with nothing on the wire. INTERIM: RETIRED by W2-P4 when its intents replace the refusal (the key and this row are deleted then). |
